...
export function registerWarlockTests(context) {
  ...
  const openActorStudio = async (name = `Quench Warlock Test ${Date.now()}`) => {
  ...
  const ensureSubclassChoiceIfRequired = async () => {
    // For warlock, no subclass choice is required at these levels
    return true;
  };
  ...
  const shouldExpectWarlockSpellsForLevel = (level) => {
    // Warlock spellcasting rules: gets spells at level 1
    return level >= 1;
  };
  ...
  describe('Warlock', function () {
    ...
    const warlockClassUuid = 'Compendium.dnd-players-handbook.classes.Item.phbclWarlock000';
    ...
    it('should auto-run warlock creation and handle spell step as required by rules', async function () {
      ...
      assertCoreSelectionsOnActor(warlockActor, {
        classIdentifier: 'warlock',
        classUuid: warlockClassUuid,

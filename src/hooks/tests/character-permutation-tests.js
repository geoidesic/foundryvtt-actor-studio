...
export function registerCharacterPermutationTests(context, options = {}) {
  ...
  describe('Paladin', function () {
    ...
    it('should auto-run paladin creation and handle spell step as required by rules', async function () {
      ...
      const paladinClassUuid = await resolveClassUuidByIdentifier('paladin');
      assert.ok(paladinClassUuid, 'Paladin class UUID should be resolvable from dnd-players-handbook.classes');
      ...
      if (!creationClosedEarly) {
        ...
        if (spellsTabAppeared) {
          const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 30000 });
          assert.ok(spellStepResult.visible, 'Spells tab should be detectable when shown during paladin creation');
          assert.ok(spellStepResult.completed, 'Paladin creation spells step should complete via UI controls when displayed');
        }
      }
    ...
    it('should level paladin from 1 to 2 and handle spells according to rules', async function () {
      ...
      const expectSpellsAtLevel2 = shouldExpectPaladinSpellsForLevel(2);
      if (expectSpellsAtLevel2) {
        const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 25000 });
        assert.ok(spellStepResult.visible, 'Spells tab should appear for paladin level 2 in this ruleset');
        assert.ok(spellStepResult.completed, 'Paladin level 1->2 spells step should complete (finalize or skip)');
      } else {
        const spellsTabAppeared = await waitForCondition(() => hasSpellsTabVisible(), 15000, 200);
        assert.ok(!spellsTabAppeared, 'Spells tab should not appear for paladin level 1->2 in this ruleset');
      }
    ...
    it('should level paladin from 2 to 3 and handle spells according to rules', async function () {
      ...
      const expectSpellsAtLevel3 = shouldExpectPaladinSpellsForLevel(3);
      if (expectSpellsAtLevel3) {
        const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 25000 });
        assert.ok(spellStepResult.visible, 'Spells tab should appear for paladin level 3 in this ruleset');
        assert.ok(spellStepResult.completed, 'Paladin level 2->3 spells step should complete (finalize or skip)');
      } else {
        const spellsTabAppeared = await waitForCondition(() => hasSpellsTabVisible(), 15000, 200);
        assert.ok(!spellsTabAppeared, 'Spells tab should not appear for paladin level 2->3 in this ruleset');
      }

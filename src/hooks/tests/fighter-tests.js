...
export function registerFighterTests(context) {
  ...
  const openActorStudio = async (name = `Quench Fighter Test ${Date.now()}`) => {
  ...
  const ensureSubclassChoiceIfRequired = async () => {
    // For fighter, no subclass choice is required at these levels
    return true;
  };
  ...
  describe('Fighter', function () {
    ...
    it('should auto-run fighter creation without spell selection', async function () {
      ...
      assertCoreSelectionsOnActor(fighterActor, {
        classIdentifier: 'fighter',
        classUuid: fighterClassUuid,

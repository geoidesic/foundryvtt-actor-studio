...
export function registerArtificerTests(context) {
  ...
  const openActorStudio = async (name = `Quench Artificer Test ${Date.now()}`) => {
  ...
  const ensureSubclassChoiceIfRequired = async () => {
    // For artificer, no subclass choice is required at these levels
    return true;
  };
  ...
  const shouldExpectArtificerSpellsForLevel = (level) => {
    // Artificer spellcasting rules: gets spells at level 1
    return level >= 1;
  };
  ...
  describe('Artificer', function () {
    let savedDebugState = {};
    let originalMilestoneLeveling = false;
    let originalForceTakeAverageHitPoints = false;
    let artificerActorId = null;
    let artificerActor = null;
    const testActorName = `Quench Artificer Automation ${Date.now()}`;
    const artificerClassUuid = 'Compendium.dnd-tashas-cauldron.tcoe-character-options.Item.tcoeArtificer000';
    ...
    const findArtificerActorByName = () => {
    ...
    const getCurrentArtificerActor = () => {
      if (artificerActorId) {
        const actorFromId = game.actors.get(artificerActorId) || null;
        if (actorFromId) return actorFromId;
      }

      const actorFromName = findArtificerActorByName();
      if (actorFromName && !artificerActorId) {
        artificerActorId = actorFromName.id;
      }
    ...
    after(async function () {
      ...
      const actorToCleanup = artificerActorId ? game.actors.get(artificerActorId) : (artificerActor || findArtificerActorByName());
    ...
    it('should auto-run artificer creation and handle spell step as required by rules', async function () {
      this.timeout(TEST_TIMEOUTS.perTest);
      ...
      configureQuenchAutomation({
        selections: {
          ...
          characterClass: artificerClassUuid
        }
      });

      await closeActorStudioIfOpen();
      const creationApp = await openActorStudio(testActorName);
      assert.ok(creationApp, 'Actor Studio should open for artificer creation test');

      const requiredTabsVisited = await visitRequiredCreationTabs();
      assert.ok(requiredTabsVisited, 'All visible creation tabs should be visited before creating the artificer');
      ...
      assert.ok(createClicked, 'Create Character button should be clickable for artificer creation');
      ...
      if (!creationClosedEarly) {
        ...
        if (spellsTabAppeared) {
          ...
          assert.ok(spellStepResult.visible, 'Spells tab should be detectable when shown during artificer creation');
          assert.ok(spellStepResult.completed, 'Artificer creation spells step should complete via UI controls when displayed');
        }
      }

      const actorStudioClosed = await waitForActorStudioClosed();
      assert.ok(actorStudioClosed, 'Actor Studio should close after artificer creation completes');

      await wait(1200);
      artificerActorId = get(actorInGame)?.id || null;
      if (!artificerActorId) {
        const actorByName = findArtificerActorByName();
        artificerActorId = actorByName?.id || null;
      }
      assert.ok(artificerActorId, 'Created artificer actor id should be available after creation');

      const reachedLevel1 = await waitForActorClassLevel(artificerActorId, 'artificer', 1);
      assert.ok(reachedLevel1, 'Artificer class level should be embedded at level 1 after creation');

      artificerActor = game.actors.get(artificerActorId);
      assert.ok(artificerActor, 'Created artificer actor should exist in world actors');

      const hasBackground = artificerActor.items.some((item) => item.type === 'background');
      assert.ok(hasBackground, 'Created artificer actor should have a background item embedded');

      assertCoreSelectionsOnActor(artificerActor, {
        classIdentifier: 'artificer',
        classUuid: artificerClassUuid,
    ...
    it('should open level-up app from artificer actor sheet when milestone leveling is enabled', async function () {
      ...
      const actor = getCurrentArtificerActor();
      assert.ok(actor, 'Existing created artificer actor should be available for level-up tests');
      ...
      assert.ok(levelUpButtonClicked, 'Actor Studio level-up button should be clickable on the artificer actor sheet');

      const levelUpAppOpened = await waitForLevelUpAppOpen();
      assert.ok(levelUpAppOpened, 'Level-up Actor Studio app should open from artificer actor sheet button');

      await closeActorStudioIfOpen();
    });

    it('should level artificer from 1 to 2 and handle spells according to rules', async function () {
      this.timeout(TEST_TIMEOUTS.perTest);

      const actor = getCurrentArtificerActor();
      assert.ok(actor, 'Existing created artificer actor should be available for 1->2 level-up test');
      ...
      const levelUpButtonClicked = await clickActorStudioLevelUpButtonOnSheet(actor);
      assert.ok(levelUpButtonClicked, 'Level-up button should be clickable for artificer 1->2 test');

      const levelUpAppOpened = await waitForLevelUpAppOpen();
      assert.ok(levelUpAppOpened, 'Level-up app should open for artificer 1->2 test');

      const levelUpTabFocused = await focusLevelUpClassTab();
      assert.ok(levelUpTabFocused, 'Level-up/Class tab should be focused before artificer 1->2 class selection');

      const subclassHandled = await ensureSubclassChoiceIfRequired();
      assert.ok(subclassHandled, 'Subclass choice should be selected when required before artificer 1->2 progression');

      const plusClicked = await clickExistingClassPlusInLevelUp();
      assert.ok(plusClicked, 'Existing class plus button should be clicked to select artificer level-up');
      ...
      assert.ok(addLevelClicked, 'Add Level button should be clickable for artificer 1->2 progression');

      const expectSpellsAtLevel2 = shouldExpectArtificerSpellsForLevel(2);
      if (expectSpellsAtLevel2) {
        const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 25000 });
        assert.ok(spellStepResult.visible, 'Spells tab should appear for artificer level 2 in this ruleset');
        assert.ok(spellStepResult.completed, 'Artificer level 1->2 spells step should complete (finalize or skip)');
      } else {
        const spellsTabAppeared = await waitForCondition(() => hasSpellsTabVisible(), 15000, 200);
        assert.ok(!spellsTabAppeared, 'Spells tab should not appear for artificer level 1->2 in this ruleset');
      }
      ...
      const reachedLevel2 = await waitForActorClassLevel(artificerActorId, 'artificer', 2);
      assert.ok(reachedLevel2, 'Artificer class level should progress to 2');

      await closeActorStudioIfOpen();
    });

    it('should level artificer from 2 to 3 and handle spells according to rules', async function () {
      this.timeout(TEST_TIMEOUTS.perTest);

      const actor = getCurrentArtificerActor();
      assert.ok(actor, 'Existing created artificer actor should be available for 2->3 level-up test');
      ...
      const levelUpButtonClicked = await clickActorStudioLevelUpButtonOnSheet(actor);
      assert.ok(levelUpButtonClicked, 'Level-up button should be clickable for artificer 2->3 test');

      const levelUpAppOpened = await waitForLevelUpAppOpen();
      assert.ok(levelUpAppOpened, 'Level-up app should open for artificer 2->3 test');

      const levelUpTabFocused = await focusLevelUpClassTab();
      assert.ok(levelUpTabFocused, 'Level-up/Class tab should be focused before artificer 2->3 class selection');

      const subclassHandled = await ensureSubclassChoiceIfRequired();
      assert.ok(subclassHandled, 'Subclass choice should be selected when required before artificer 2->3 progression');

      const plusClicked = await clickExistingClassPlusInLevelUp();
      assert.ok(plusClicked, 'Existing class plus button should be clicked for artificer 2->3 test');
      ...
      assert.ok(addLevelClicked, 'Add Level button should be clickable for artificer 2->3 progression');

      const expectSpellsAtLevel3 = shouldExpectArtificerSpellsForLevel(3);
      if (expectSpellsAtLevel3) {
        const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 25000 });
        assert.ok(spellStepResult.visible, 'Spells tab should appear for artificer level 3 in this ruleset');
        assert.ok(spellStepResult.completed, 'Artificer level 2->3 spells step should complete (finalize or skip)');
      } else {
        const spellsTabAppeared = await waitForCondition(() => hasSpellsTabVisible(), 15000, 200);
        assert.ok(!spellsTabAppeared, 'Spells tab should not appear for artificer level 2->3 in this ruleset');
      }
      ...
      const reachedLevel3 = await waitForActorClassLevel(artificerActorId, 'artificer', 3);
      assert.ok(reachedLevel3, 'Artificer class level should progress to 3');

      artificerActor = getCurrentArtificerActor();
      await closeActorStudioIfOpen();
    });
  });
}
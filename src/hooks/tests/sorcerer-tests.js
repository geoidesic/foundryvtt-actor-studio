...
export function registerSorcererTests(context) {
  ...
  const openActorStudio = async (name = `Quench Sorcerer Test ${Date.now()}`) => {
  ...
  const ensureSubclassChoiceIfRequired = async () => {
    // For sorcerer, no subclass choice is required at these levels
    return true;
  };
  ...
  const shouldExpectSorcererSpellsForLevel = (level) => {
    // Sorcerer spellcasting rules: full spellcaster, gets spells at level 1
    return level >= 1;
  };
  ...
  describe('Sorcerer', function () {
    let savedDebugState = {};
    let originalMilestoneLeveling = false;
    let originalForceTakeAverageHitPoints = false;
    let sorcererActorId = null;
    let sorcererActor = null;
    const testActorName = `Quench Sorcerer Automation ${Date.now()}`;
    const sorcererClassUuid = 'Compendium.dnd-players-handbook.classes.Item.phbscrSorcerer00';
    ...
    const getCurrentSorcererActor = () => {
      if (sorcererActorId) {
        const actorFromId = game.actors.get(sorcererActorId) || null;
        if (actorFromId) return actorFromId;
      }

      const actorFromName = findSorcererActorByName();
      if (actorFromName && !sorcererActorId) {
        sorcererActorId = actorFromName.id;
      }
    ...
    after(async function () {
      ...
      const actorToCleanup = sorcererActorId ? game.actors.get(sorcererActorId) : (sorcererActor || findSorcererActorByName());
    ...
    it('should auto-run sorcerer creation and handle spell step as required by rules', async function () {
      this.timeout(TEST_TIMEOUTS.perTest);
      ...
      configureQuenchAutomation({
        selections: {
          ...
          characterClass: sorcererClassUuid
        }
      });

      await closeActorStudioIfOpen();
      const creationApp = await openActorStudio(testActorName);
      assert.ok(creationApp, 'Actor Studio should open for sorcerer creation test');

      const requiredTabsVisited = await visitRequiredCreationTabs();
      assert.ok(requiredTabsVisited, 'All visible creation tabs should be visited before creating the sorcerer');
      ...
      assert.ok(createClicked, 'Create Character button should be clickable for sorcerer creation');
      ...
      if (!creationClosedEarly) {
        ...
        if (spellsTabAppeared) {
          ...
          assert.ok(spellStepResult.visible, 'Spells tab should be detectable when shown during sorcerer creation');
          assert.ok(spellStepResult.completed, 'Sorcerer creation spells step should complete via UI controls when displayed');
        }
      }

      const actorStudioClosed = await waitForActorStudioClosed();
      assert.ok(actorStudioClosed, 'Actor Studio should close after sorcerer creation completes');

      await wait(1200);
      sorcererActorId = get(actorInGame)?.id || null;
      if (!sorcererActorId) {
        const actorByName = findSorcererActorByName();
        sorcererActorId = actorByName?.id || null;
      }
      assert.ok(sorcererActorId, 'Created sorcerer actor id should be available after creation');

      const reachedLevel1 = await waitForActorClassLevel(sorcererActorId, 'sorcerer', 1);
      assert.ok(reachedLevel1, 'Sorcerer class level should be embedded at level 1 after creation');

      sorcererActor = game.actors.get(sorcererActorId);
      assert.ok(sorcererActor, 'Created sorcerer actor should exist in world actors');

      const hasBackground = sorcererActor.items.some((item) => item.type === 'background');
      assert.ok(hasBackground, 'Created sorcerer actor should have a background item embedded');

      assertCoreSelectionsOnActor(sorcererActor, {
        classIdentifier: 'sorcerer',
        classUuid: sorcererClassUuid,
    ...
    it('should open level-up app from sorcerer actor sheet when milestone leveling is enabled', async function () {
      ...
      const actor = getCurrentSorcererActor();
      assert.ok(actor, 'Existing created sorcerer actor should be available for level-up tests');
      ...
      assert.ok(levelUpButtonClicked, 'Actor Studio level-up button should be clickable on the sorcerer actor sheet');

      const levelUpAppOpened = await waitForLevelUpAppOpen();
      assert.ok(levelUpAppOpened, 'Level-up Actor Studio app should open from sorcerer actor sheet button');

      await closeActorStudioIfOpen();
    });

    it('should level sorcerer from 1 to 2 and handle spells according to rules', async function () {
      this.timeout(TEST_TIMEOUTS.perTest);

      const actor = getCurrentSorcererActor();
      assert.ok(actor, 'Existing created sorcerer actor should be available for 1->2 level-up test');
      ...
      const levelUpButtonClicked = await clickActorStudioLevelUpButtonOnSheet(actor);
      assert.ok(levelUpButtonClicked, 'Level-up button should be clickable for sorcerer 1->2 test');

      const levelUpAppOpened = await waitForLevelUpAppOpen();
      assert.ok(levelUpAppOpened, 'Level-up app should open for sorcerer 1->2 test');

      const levelUpTabFocused = await focusLevelUpClassTab();
      assert.ok(levelUpTabFocused, 'Level-up/Class tab should be focused before sorcerer 1->2 class selection');

      const subclassHandled = await ensureSubclassChoiceIfRequired();
      assert.ok(subclassHandled, 'Subclass choice should be selected when required before sorcerer 1->2 progression');

      const plusClicked = await clickExistingClassPlusInLevelUp();
      assert.ok(plusClicked, 'Existing class plus button should be clicked to select sorcerer level-up');
      ...
      assert.ok(addLevelClicked, 'Add Level button should be clickable for sorcerer 1->2 progression');

      const expectSpellsAtLevel2 = shouldExpectSorcererSpellsForLevel(2);
      if (expectSpellsAtLevel2) {
        const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 25000 });
        assert.ok(spellStepResult.visible, 'Spells tab should appear for sorcerer level 2 in this ruleset');
        assert.ok(spellStepResult.completed, 'Sorcerer level 1->2 spells step should complete (finalize or skip)');
      } else {
        const spellsTabAppeared = await waitForCondition(() => hasSpellsTabVisible(), 15000, 200);
        assert.ok(!spellsTabAppeared, 'Spells tab should not appear for sorcerer level 1->2 in this ruleset');
      }
      ...
      const reachedLevel2 = await waitForActorClassLevel(sorcererActorId, 'sorcerer', 2);
      assert.ok(reachedLevel2, 'Sorcerer class level should progress to 2');

      await closeActorStudioIfOpen();
    });

    it('should level sorcerer from 2 to 3 and handle spells according to rules', async function () {
      this.timeout(TEST_TIMEOUTS.perTest);

      const actor = getCurrentSorcererActor();
      assert.ok(actor, 'Existing created sorcerer actor should be available for 2->3 level-up test');
      ...
      const levelUpButtonClicked = await clickActorStudioLevelUpButtonOnSheet(actor);
      assert.ok(levelUpButtonClicked, 'Level-up button should be clickable for sorcerer 2->3 test');

      const levelUpAppOpened = await waitForLevelUpAppOpen();
      assert.ok(levelUpAppOpened, 'Level-up app should open for sorcerer 2->3 test');

      const levelUpTabFocused = await focusLevelUpClassTab();
      assert.ok(levelUpTabFocused, 'Level-up/Class tab should be focused before sorcerer 2->3 class selection');

      const subclassHandled = await ensureSubclassChoiceIfRequired();
      assert.ok(subclassHandled, 'Subclass choice should be selected when required before sorcerer 2->3 progression');

      const plusClicked = await clickExistingClassPlusInLevelUp();
      assert.ok(plusClicked, 'Existing class plus button should be clicked for sorcerer 2->3 test');
      ...
      assert.ok(addLevelClicked, 'Add Level button should be clickable for sorcerer 2->3 progression');

      const expectSpellsAtLevel3 = shouldExpectSorcererSpellsForLevel(3);
      if (expectSpellsAtLevel3) {
        const spellStepResult = await completeSpellsStepIfVisible({ timeoutMs: 25000 });
        assert.ok(spellStepResult.visible, 'Spells tab should appear for sorcerer level 3 in this ruleset');
        assert.ok(spellStepResult.completed, 'Sorcerer level 2->3 spells step should complete (finalize or skip)');
      } else {
        const spellsTabAppeared = await waitForCondition(() => hasSpellsTabVisible(), 15000, 200);
        assert.ok(!spellsTabAppeared, 'Spells tab should not appear for sorcerer level 2->3 in this ruleset');
      }
      ...
      const reachedLevel3 = await waitForActorClassLevel(sorcererActorId, 'sorcerer', 3);
      assert.ok(reachedLevel3, 'Sorcerer class level should progress to 3');

      sorcererActor = getCurrentSorcererActor();
      await closeActorStudioIfOpen();
    });
  });
}